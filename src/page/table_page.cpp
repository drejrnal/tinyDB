 ...
 */
    void TablePage::Init(page_id_t page_id, size_t page_size,
        ...
        if (ENABLE_LOGGING) {
            ...
            //NEW PAGE log record
            LogRecord logRecord(txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::NEWPAGE,
                                prev_page_id, page_id );
            lsn_t current_lsn = log_manager->AppendLogRecord( logRecord );
 ...
 */
    bool TablePage::InsertTuple(const Tuple &tuple, RID &rid, Transaction *txn,
        ...
        if (ENABLE_LOGGING) {
            ...
            LogRecord logRecord( txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::INSERT,
                                 rid, tuple);
            lsn_t lsn = log_manager->AppendLogRecord( logRecord );
            txn->SetPrevLSN(lsn);
            SetLSN( lsn );
        }
 ...
 */
    bool TablePage::MarkDelete(const RID &rid, Transaction *txn,
        ...
        if (ENABLE_LOGGING) {
            ...
            // TODO: add your logging logic here
            Tuple tuple;
            LogRecord logRecord( txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::MARKDELETE,
                                 rid, tuple );
            lsn_t lsn = log_manager->AppendLogRecord( logRecord );
            txn->SetPrevLSN(lsn);
            SetLSN( lsn );
    ...
    bool TablePage::UpdateTuple(const Tuple &new_tuple, Tuple &old_tuple,
        ...
        /*
         * 在MarkDelete时 将tuple size置为小于0的数，所以更新时需要判断
         */
        if (tuple_size <= 0) {
            if (ENABLE_LOGGING) {
                txn->SetState( TransactionState::ABORTED);
        ...
        if (ENABLE_LOGGING) {
            ...
            // TODO: add your logging logic here
            // update log record
            LogRecord logRecord( txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::UPDATE,
    ...
    void TablePage::ApplyDelete(const RID &rid, Transaction *txn,
        ...
        if (ENABLE_LOGGING) {
            ...
            assert(txn->GetExclusiveLockSet()->find(rid) !=
                   txn->GetExclusiveLockSet()->end());
            // TODO: add your logging logic here
            //Apply delete的log record写入log buffer中
            LogRecord logRecord( txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::APPLYDELETE,
                                 rid, delete_tuple );
            lsn_t lsn = log_manager->AppendLogRecord( logRecord );
            txn->SetPrevLSN(lsn);
            SetLSN( lsn );
 ...
 */
    void TablePage::RollbackDelete(const RID &rid, Transaction *txn,
        ...
        if (ENABLE_LOGGING) {
            ...
            // 执行aborting a txn时，该txn持有的写锁并未释放
            assert(txn->GetExclusiveLockSet()->find(rid) !=
                   txn->GetExclusiveLockSet()->end());
            //make sure your log records are permanently stored on disk file before release the locks
            // TODO: add your logging logic here
            /*
             * 往日志中写入类型为ROLL_BACK_DELETE的log record
             */
            Tuple tuple;
            LogRecord logRecord( txn->GetTransactionId(), txn->GetPrevLSN(), LogRecordType::ROLLBACKDELETE,
                                 rid, tuple );
            lsn_t lsn = log_manager->AppendLogRecord( logRecord );
            txn->SetPrevLSN(lsn);
            SetLSN( lsn );
    ...
    bool TablePage::GetTuple(const RID &rid, Tuple &tuple, Transaction *txn,
        ...
        int slot_num = rid.GetSlotNum();
        /*
         * 判断tuple(rid标识)是否合法，即该tuple是否存在于slotted page内部
         */
        if (slot_num >= GetTupleCount()) {
            if (ENABLE_LOGGING)
                txn->SetState(TransactionState::ABORTED);
        ...
        int32_t tuple_size = GetTupleSize(slot_num); //从table page header内找到该tuple的大小
        if (tuple_size <= 0) {
            if (ENABLE_LOGGING)
                txn->SetState(TransactionState::ABORTED);
            return false;
